...
`./build.sh tizen_tm1 USR` command로 kernel을 build하고, sdb를 이용해 생성된 tar파일을 기기에 Push한 후
reboot후 sdcard를 사용하여 flash하면 새로운 kernel을 사용할 수 있다.

**2.High-level design and impelementation**

 프로젝트의 목표가 process tree를 dfs로 탐색하여 user가 원하는 만큼 prinfo를 복사하여
 ...
 ```
  ...
  user에게서 prinfo potinter와 int pointer를 받아온다. 다만 kernel에서 함부로 user의 memory space를
  변경하거나 참조하는 것은 위험하기 때문에 buf와 nr을 그대로 쓰지 않고 리눅스에서 주어진 매크로를
  사용하여 user에게서 값을 얻어오고 user로 값을 복사한다. 그러한 과정에서 error가 났다면 적절한 error value를
  리턴하도록 하였다. 또한 dfs를 하는 도중 task_struct list가 변경되는 것을 막기 위해 lock을 걸어주었다.
  
  **Error handling**  
  System call에서 error handling은 발생하는 error를  errno-base.h에서 error에 해당하는 값을 찾아
 -를 붙여서 리턴하는 방식으로 이루어 진다.
```c
if (buf == NULL || nr == NULL)
	return -EINVAL;
```
prinfo를 담을 buf나 최대 process수인 nr이 NULL이면 invalid argument인 -EINVAL을 리턴한다
```c
if (knr < 1)
	return -EINVAL;
```
...
user영역에서 nr 을 불러오거나, kbuf를 user 영역으로 보내는데 실패하면 -EFAULT를 리턴한다
  ...
  ```
   dfs함수에서 visit이 불리면 kbuf로 task_struct의 data를 복사하도록 구현하였다.
   전체 process 개수를 리턴해야 하므로 *i는 visit할 때마다 증가하지만, *knr과 같아지는
   순간 data의 복사는 일어나지 않는다.
...
**3.Investigation of the process tree**	 
 ...
 pid가 2씩 증가 할 때마다 systemd-udevd의 child process가 생성되고 있었다.그 child process가
 pid를 먼저 차지해서 test의 pid가 2 증가하게 된것이다. systemd-udevd는 kernel로 부터 uevent를
 받아서 device에 알맞은 instruction을 수행하는 daemon process이다.왜 주기적으로
 systemd-udevd의 child process가 생성되는지 알아보기 위해`udevadm monitor` command를 사용해서
 다음과 같은 결과를 얻었다.  
...
....
```
 일정시간마다 udev monitor에 mesg가 나오는 것으로 보아 kernel이 cellphone의 battery를 확인하기 위한 event를 udev에게 보내면 그 때마다 udev의 child process가 생성되고 있었음을 알 수 있다.  
 ...
 **Discuss**  
 ...
 아니고 중간 sibling으로 존재 하는 것으로 보아 launchpad-loader중 하나가 execev해서 Application이 되고,
